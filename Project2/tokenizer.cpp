...
bool Tokenizer::isKeyword(const std::string& str) {
    static const std::unordered_set<std::string> keywords = {
        "function", "procedure", "if", "else", "for", "while", "return",
        "char", "int", "bool", "TRUE", "FALSE", "void", "main", "printf",
        "getchar"
    };
...
Token Tokenizer::identifierOrKeyword() {
    ...
    if (isKeyword(lexeme)) {
        TokenType type;
        if (lexeme == "function") type = TokenType::FUNCTION;
        else if (lexeme == "procedure") type = TokenType::PROCEDURE;
        ...
        else if (lexeme == "int") type = TokenType::INT;
        ...
        else if (lexeme == "main") type = TokenType::MAIN;
        else if (lexeme == "printf") type = TokenType::PRINTF;
        else if (lexeme == "getchar") type = TokenType::GETCHAR;
        else type = TokenType::IDENTIFIER; // Fallback
...
Token Tokenizer::stringLiteral() {
    ...
    if (currentChar == quoteType) {
        ...
        advance();
        TokenType type = (quoteType == '"') ? TokenType::STRING : TokenType::CHAR_LITERAL;
        return Token(type, lexeme, startLine);
    } else {
        // Unterminated string
        return Token(TokenType::INVALID_TOKEN, lexeme, startLine);
...
Token Tokenizer::operatorOrDelimiter() {
    ...
    if (currentChar == '=' || currentChar == '!' || currentChar == '<' || currentChar == '>') {
        ...
        if (currentChar == '=') {
            lexeme += currentChar;
            advance();
            if (lexeme == "==") return Token(TokenType::BOOLEAN_EQUAL, lexeme, startLine);
            if (lexeme == "!=") return Token(TokenType::BOOLEAN_NOT_EQUAL, lexeme, startLine);
            if (lexeme == "<=") return Token(TokenType::LT_EQUAL, lexeme, startLine);
            if (lexeme == ">=") return Token(TokenType::GT_EQUAL, lexeme, startLine);
        } else {
            // Single-character operators
            if (lexeme == "=") return Token(TokenType::ASSIGNMENT_OPERATOR, lexeme, startLine);
            if (lexeme == "!") return Token(TokenType::BOOLEAN_NOT, lexeme, startLine);
            if (lexeme == "<") return Token(TokenType::LT, lexeme, startLine);
            if (lexeme == ">") return Token(TokenType::GT, lexeme, startLine);
        ...
        if (currentChar == firstChar) {
            lexeme += currentChar;
            advance();
            if (lexeme == "&&") return Token(TokenType::BOOLEAN_AND, lexeme, startLine);
            if (lexeme == "||") return Token(TokenType::BOOLEAN_OR, lexeme, startLine);
        } else {
            // Invalid operator
            return Token(TokenType::INVALID_TOKEN, lexeme, startLine);
        }
        ...
        switch (firstChar) {
            ...
            case '\'': type = TokenType::SINGLE_QUOTE; break;
            case '"': type = TokenType::DOUBLE_QUOTE; break;
            default: type = TokenType::INVALID_TOKEN; break;
        }
        advance();
...
Token Tokenizer::getNextToken() {
    ...
    if (currentChar == '"' || currentChar == '\'') {
        ...
        TokenType quoteTokenType = (quoteType == '"') ? TokenType::DOUBLE_QUOTE : TokenType::SINGLE_QUOTE;
        Token openingQuoteToken(quoteTokenType, std::string(1, quoteType), startLine);
        ...
        if (currentChar == quoteType) {
            ...
            TokenType contentTokenType = (quoteType == '"') ? TokenType::STRING : TokenType::CHAR_LITERAL;
            Token contentToken(contentTokenType, lexeme, startLine);

            // Create token for closing quote
            Token closingQuoteToken(quoteTokenType, std::string(1, quoteType), lineNumber);
...
std::vector<Token> Tokenizer::tokenize() {
    ...
    while (token.type != TokenType::END_OF_FILE) {
        if (token.type == TokenType::INVALID_TOKEN) {
            ...
            std::string errorMessage = "Syntax error on line " + std::to_string(token.lineNumber) +
